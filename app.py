...
# Minimal HTTP app for automated tests/health checks.
app = Flask(__name__)


@app.route("/")
def home():
    return "ACEest Fitness & Performance", 200
...
if __name__ == "__main__":
    if not all([tk, ttk, messagebox, plt, FigureCanvasTkAgg, FPDF]):
        raise RuntimeError(
            "GUI dependencies are missing. Install tkinter/matplotlib/fpdf for desktop mode, "
            "or run the Flask app with: flask --app app run --host=0.0.0.0 --port=5000"
        )
    init_db()
    root = tk.Tk()
    gui_app = ACEestApp(root)
    root.mainloop()
